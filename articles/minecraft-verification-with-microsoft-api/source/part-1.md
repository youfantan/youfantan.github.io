...
为了保证用户的游戏体验~~（担心自己总部被Dungeons玩家拆掉）~~，Mojang把Dungeons启动的模块写到Minecraft Launcher里了~~Mojang终于会一个启动器里写两份验证代码了~~，现在打开Minecraft Launcher后可以Switch Account到Microsoft Account并启动Minecraft Dungeons。但来回切换很麻烦，于是Mojang就扔掉了自家的祖传Yggdrasil换成了Microsoft OAuth~~未曾设想的道路~~。
